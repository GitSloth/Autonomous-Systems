...
if VIDEO:
    ...
    # cap = cv2.VideoCapture('http://145.24.238.27:8080/video') # ip phone camera stream
...
while(True):
    ...
    if VIDEO:
        ...
        hsv_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV) #cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    ...
    green_leds = [cnt for cnt in contours_green if cv2.contourArea(cnt) > 10]
    red_leds = [cnt for cnt in contours_red if cv2.contourArea(cnt) > 10]


    if VIDEO:
        #contours
        frame_with_contours = frame.copy()
        cv2.drawContours(frame_with_contours, green_leds, -1, (0, 255, 0), 2)
        cv2.drawContours(frame_with_contours, red_leds, -1, (0, 0, 255), 2)
