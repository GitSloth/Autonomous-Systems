...
const char *MQTT_CLIENT_ID = "esp_robot_3"; 
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  else if(robot_message == "web_robot_3:dire:backwards"){
    digitalWrite(lamp_forward, LOW);
    digitalWrite(lamp_backward, HIGH);
    digitalWrite(lamp_left, LOW);
    digitalWrite(lamp_right, LOW);
  }
   else if(robot_message == "web_robot_3:dire:left"){
    digitalWrite(lamp_forward, LOW);
    digitalWrite(lamp_backward, LOW);
    digitalWrite(lamp_left, HIGH);
    digitalWrite(lamp_right, LOW);
  }
   else if(robot_message == "web_robot_3:dire:right"){
    digitalWrite(lamp_forward, LOW);
    digitalWrite(lamp_backward, LOW);
    digitalWrite(lamp_left, LOW);
    digitalWrite(lamp_right, HIGH);
  }
   else if(robot_message == "web_robot_3:dire:off"){
    digitalWrite(lamp_forward, LOW);
    digitalWrite(lamp_backward, LOW);
    digitalWrite(lamp_left, LOW);
    digitalWrite(lamp_right, LOW);
...
void emergency(){
  ...
  client.publish("publish/robots",  "esp_robot_3:emgc:on");
